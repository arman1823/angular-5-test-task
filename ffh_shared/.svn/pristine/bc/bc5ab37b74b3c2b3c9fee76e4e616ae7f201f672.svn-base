        ...
        (function (xlsx) {
            ...
            var FlexGridXlsxConverter = /** @class */ (function () {
                ...
                FlexGridXlsxConverter._parseFlexGridRowToSheetRow = function (panel, workbookRow, rowIndex, startColIndex, columnSettings, includeCellStyles, fakeCell, isGroupRow, groupLevel, includeColumns, formatItem) {
                    var flex, row, columnSetting, ci, orgFormat, format, val, unformattedVal, groupHeader, isFormula, formula, cellIndex, cellStyle, mergedCells, rowSpan, colSpan, sheetInfo, valIsDate, isCommonRow = false, bcol, isStartMergedCell, recordIndex, xlsxCell, getFormattedCell, xlsxFormatItemArgs, cloneFakeCell, parsedCellStyle, hAlign, indent;
                    flex = panel.grid;
                    ...
                    for (ci = 0; ci < panel.columns.length; ci++) {
                        ...
                        if (panel === flex.cells && isGroupRow && row['hasChildren'] && ci === flex.columns.firstVisibleIndex) {
                            ...
                            xlsxCell = {
                                value: isFormula ? null : format === 'General' ? (wijmo.isString(val) ? val.replace(/^(\')(\s*=)/, '$2') : val) : unformattedVal,
                                isDate: valIsDate,
                                formula: isFormula ? this._parseToExcelFormula(val, valIsDate) : null,
