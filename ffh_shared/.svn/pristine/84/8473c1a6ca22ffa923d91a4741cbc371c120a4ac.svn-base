    ...
    function moveFocus(parent, offset) {
        ...
        for (var i = 0; i < elements.length; i++) {
            ...
            if (el.offsetHeight > 0 && // visible, (clientHeight is zero for A tags, TFS 238460)
                ...
                if (wijmo.Control.getControl(el) || !el.querySelector(tags)) {
                    focusable.push(el);
                }
            }
        }
    ...
    var Control = /** @class */ (function () {
        ...
        Control.prototype.containsFocus = function () {
            ...
            for (var e = ae; e; e = e.parentElement) {
                ...
                if (owner && wijmo.contains(this._e, owner)) {
                    return true;
                }
            }
            // see if active element is in a popup control
            // and the owner is contained in this control
            var popup = Control.getControl(wijmo.closest(ae, '.wj-control.wj-popup'));
        ...
        Control.prototype._updateFocusState = function () {
            ...
            // before moving it to the new focused element
            // (TFS 255078, 255011, 254885, 254865, 250941, 146949)
            setTimeout(function () {
                // travel up the tree
                    ...
                    if (ctl) {
                        ...
                        if (focus_1 != ctl._focus) {
                            ...
                            if (!focus_1) {
                                var owner = e[Control._OWNR_KEY];
                                if (owner) {
                                    var ctl_1 = Control.getControl(wijmo.closest(owner, '.wj-control'));
                                    if (ctl_1) {
        ...
        Control._rxInputAtts = /name|tabindex|placeholder|autofocus|autocomplete|autocorrect|autocapitalize|spellcheck|readonly|minlength|maxlength|pattern/i;
        return Control;
    }());
    ...
    var Clipboard = /** @class */ (function () {
        ...
        Clipboard._copyPasteInternal = function (textOrCallback) {
            ...
            if (parent) {
                ...
                if (wijmo.isString(textOrCallback)) {
                ...
                setTimeout(function () {
                    ...
                    if (wijmo.isFunction(textOrCallback)) {
                        textOrCallback(text);
                    }
...
(function (wijmo) {
    ...
    function showPopup(popup, ref, above, fadeIn, copyStyles) {
        ...
        var pos = new wijmo.Point(), rc = null, scrWid = document.documentElement.clientWidth, //innerWidth,
        scrHei = document.documentElement.clientHeight; //innerHeight;
        if (ref && ref.clientX != null && ref.clientY != null && ref.pageX != null && ref.pageY != null) {
            if (ref.clientX <= 0 && ref.clientY <= 0 && ref.target) {
                ...
                // use page*-page*Offset instead of client*, which should be the same
                // but gives wrong results in some scenarios (e.g. pinch-zoomed Chrome / Android)
        ...
        // handle scroll offset and pinch zoom (TFS 202906)
        var position = 'absolute', rcBody = document.body.getBoundingClientRect(), ptOffset = new wijmo.Point(Math.max(0, -rcBody.left), Math.max(0, -rcBody.top));
        if (parent != document.body) {
            var op = popup.offsetParent, rcParent = op.getBoundingClientRect();
            ptOffset = new wijmo.Point(op.scrollLeft - rcParent.left, op.scrollTop - rcParent.top);
        }
        ...
        if (remove && popup.parentElement) {
            setTimeout(function () {
                if (popup.style.display == 'none') {
                    var parent_1 = popup.parentElement;
                    if (parent_1) {
                        parent_1.removeChild(popup);
                    }
                }
            }, 50);
        }
