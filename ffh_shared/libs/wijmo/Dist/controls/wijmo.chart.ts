...
module wijmo.chart {
            ...
            if (extension === 'svg') {
                ...
                image.onload = () => {
                    var canvas = document.createElement('canvas'),
                        ...
                        rect = (<any>wijmo).getElementRect(node),
                        uri;
        ...
        */
        ...
        _render(engine: IRenderEngine, applyElement = true) {
            ...
            if (w > 0 && h > 0) {
                ...
                this._rectChart = new Rect(0, 0, w, h);
                this._prepareRender();
        ...
        _renderHeader(engine: IRenderEngine, rect: Rect) {
        ...
        _renderFooter(engine: IRenderEngine, rect: Rect) {
        ...
        _renderLegends(engine: IRenderEngine, rect: Rect) {
        ...
        _toControl(pt: any, y?: number): Point {
            if (isNumber(pt) && isNumber(y)) { // accept hitTest(x, y) as well
                ...
                pt = mouseToPage(pt);// new Point(pt.pageX, pt.pageY);
            }
            asType(pt, Point);
    ...
    class ExportHelper {
        ...
        static getDataUri(ele) {
            ...
            if (ele.tagName == 'svg') {
                rect = (<any>wijmo).getElementRect(ele.parentNode || ele);
                width = rect.width || 0;
...
module wijmo.chart {
    ...
    export class FlexPie extends FlexChartBase {
        ...
        _drawSlice(engine: IRenderEngine, i: number, reversed: boolean, cx: number, cy: number, r: number, irad: number, angle: number, sweep: number) {
            ...
            if (reversed) {
                if (irad > 0) {
                    ...
                    area = new _DonutSegment(new Point(cx, cy), r, irad, angle - sweep, sweep, this.startAngle);
                    area.tag = i;
                    ...
                    area = new _PieSegment(new Point(cx, cy), r, angle - sweep, sweep, this.startAngle);
                    area.tag = i;
                    this._areas.push(area);
                }
            }
                ...
                if (irad > 0) {
                    ...
                    area = new _DonutSegment(new Point(cx, cy), r, irad, angle, sweep, this.startAngle);
                    area.tag = i;
                    this._areas.push(area);
                } else {
                    if (sweep != 0) {
                    ...
                    area = new _PieSegment(new Point(cx, cy), r, angle, sweep, this.startAngle);
                    area.tag = i;
                    this._areas.push(area);
                }
                angle += sweep;
        ...
        _drawLegendItem(engine: IRenderEngine, rect: Rect, i: number, name: string) {
            ...
            if (name != null) {
                FlexChart._renderText(engine, name.toString(), new Point(rect.left + hsym + 2 * marg, yc), 0, 1, FlexChart._CSS_LABEL);
            }
        }
    ...
    export class _PieSegment implements _IHitArea, _ISegment {
        ...
        constructor(center: Point, radius: number, angle: number, sweep: number, startAngle: number = 0) {
            this._center = center;
            this._radius = radius;
    ...
    export class _DonutSegment implements _IHitArea, _ISegment {
        ...
        constructor(center: Point, radius: number, innerRadius: number, angle: number, sweep: number, startAngle: number = 0) {
            this._center = center;
            this._radius = radius;
...
module wijmo.chart {
    ...
    export class FlexChartCore extends FlexChartBase {
        ...
        */
        ...
        private _renderLegendElements(engine: IRenderEngine, series: SeriesBase, pos: Point, p: Point, areas: any[], isVertical: boolean, width: number, height: number, colRowLen: number) {
...
module wijmo.chart {
    ...
    export class Axis implements _IAxis {
        ...
        */
        ...
        _renderRotatedText(engine: IRenderEngine, val, text: string, pos: Point, halign, valign,
                center: Point, angle: number, className: string, groupClassName?: string, style?: any) {
            if (this.itemFormatter) {
        ...
        _getFormattedItem(engine: IRenderEngine, val, text: string, pos: Point, className: string) {
            if (this.itemFormatter) {
                var pt = pos.clone();
                if (this.axisType == AxisType.X) {
        ...
        private _renderRotatedLabel(engine: IRenderEngine, val, sval: string, lpt: Point, ha, labelAngle: number, lblClass: string, isNear: boolean): boolean {
            if (this.itemFormatter) {
            ...
            if (sval) {
                ...
                var center = new Point(lpt.x, lpt.y);
                var pt = new Point(lpt.x, lpt.y);

                //if label align is not set, default rotate label's align is right
                if (!this.labelAlign) {
                    if (isNear) {
                        ha = 2;
                    } else {
                ...
                var rect = new Rect(center.x + offWidth, pt.y, sz.height + 2, sz.width);
            ...
            if (this.isCandle) {
                if (_DataInfo.isValid(open) && _DataInfo.isValid(close)) {
                    ...
                    if (rotated) {
                        ...
                        area = new _RectArea(new Rect(y0, x1, y1 - y0 || 1, x2 - x1 || 1));
                    } else {
                        engine.drawRect(x1, y0, x2 - x1 || 1, y1 - y0 || 1);
                        area = new _RectArea(new Rect(x1, y0, x2 - x1 || 1, y1 - y0 || 1));
                    }
                    area.tag = dpt;
                    this.hitTester.add(area, si);
                }
