    ...
    export class ODataCollectionView extends collections.CollectionView {
            ...
            if (item) {
                ...
                httpRequest(url, {
                    ...
                    success: (xhr) => { // update keys in the new item, refresh the view
                        let newItem = JSON.parse(xhr.response);
            ...
            if (this._tbl && !nextLink) {
                ...
                if (this.sortOnServer && this.sortDescriptions.length) {
                    ...
                    for (let i = 0; i < this.sortDescriptions.length; i++) {
                        let sd = this.sortDescriptions[i];
                        if (sort) sort += ',';
                        sort += sd.property;
        ...
        /*protected*/ _getData(nextLink?: string) {
            ...
            this._toGetData = setTimeout(() => {
                ...
                // go get the data
                let url = this._getReadUrl(nextLink);
                httpRequest(url, {
                    requestHeaders: this.requestHeaders,
                    ...
                    success: (xhr) => {
                        ...
                        let dataTypes = this.dataTypes ? this.dataTypes : this._dataTypesInferred;
                        if (dataTypes) {
        ...
        private _getSchema() {
            ...
            if (this._odv) {
                ...
                httpRequest(url, {
                    requestHeaders: this.requestHeaders,
                    success: (xhr) => {
                        let m = xhr.response.match(/<.*Version\s*=\s*"(.*)"\s*>/i),
        ...
        private _asODataValueFilter(vf: grid.filter.ValueFilter): string {
            let col = vf.column,
                fld = col.binding,
                allValues = vf._getUniqueValues(col, false),
                val = '';

            // use 'eq/or' if paging on the server (TFS 285202)
            let useEqCondition = this.pageOnServer && this.pageSize && !vf.uniqueValues;

            // use 'eq/or' condition if only a few values are checked
            if (Object.keys(vf.showValues).length < allValues.length / 2 && allValues.length > 6) {
                useEqCondition = true;
            }
            ...
            } else { // build condition with 'ne/and'
                for (let i = 0; i < allValues.length; i++) {
                    let text = allValues[i].text;
                    if (vf.showValues[text] == null) {
                        if (val) val += ' and ';
                        let value = allValues[i].value;
                        val += '(' + fld + ' ne ' + this._asODataValue(value, col.dataType) + ')';
                    }
                }
            }
        ...
        private _asODataConditionFilter(cf: grid.filter.ConditionFilter): string {
            let val = this._asODataCondition(cf, cf.condition1);
