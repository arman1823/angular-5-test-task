    ...
    (function (grid) {
        ...
        var FlexGrid = /** @class */ (function (_super) {
            ...
            Object.defineProperty(FlexGrid.prototype, "_virtualizationThreshold", {
                ...
                set: function (value) {
                    wijmo._deprecated('_virtualizationThreshold', 'virtualizationThreshold');
                    this._vt = wijmo.asNumber(value);
                },
                enumerable: true,
                configurable: true
            ...
            FlexGrid.prototype._setFocusNoScroll = function (cell) {
                ...
                var sp = this.scrollPosition, st = cell.style, pos = st.position;
            ...
            FlexGrid.prototype._getWidestRow = function (p, rowRange, col, ctx) {
                var row = 0, maxWid = 0, dataType = p.columns[col].dataType, isNumeric = dataType == wijmo.DataType.Number, isBoolean = dataType == wijmo.DataType.Boolean;
                for (var r = rowRange.row; r <= rowRange.row2; r++) {
                    if (p.rows[r].isVisible) {
                        var str = p.getCellData(r, col, true), wid = dataType == isNumeric ? str.length : ctx.measureText(str).width;
                        if (wid > maxWid) {
                            maxWid = wid;
                            row = r;
                        }
            ...
            FlexGrid.prototype._cvCurrentChanged = function (sender, e) {
                if (this.collectionView && this.selectionMode != grid.SelectionMode.None) {
    ...
    (function (grid) {
        ...
        var _EditHandler = /** @class */ (function () {
            ...
            _EditHandler.prototype._updateCell = function (cell) {
                var ht = new grid.HitTestInfo(cell, null);
                if (ht.panel) {
                    cell.classList.add(grid.FlexGrid._WJS_UPDATING); // force update
                    ht.grid.cellFactory.updateCell(ht.panel, ht.row, ht.col, cell, ht.range);
                    cell.classList.remove(grid.FlexGrid._WJS_UPDATING);
                }
            };
