...
module wijmo {
    ...
    export class Control {
        ...
        static _rxInputAtts = /name|tabindex|placeholder|autofocus|autocomplete|autocorrect|autocapitalize|spellcheck|readonly|minlength|maxlength|pattern/i;

        protected _e: HTMLElement;              // host element
        protected _orgOuter: string;            // host element's original outerHTML
        protected _orgInner: string;            // host element's original innerHTML
...
module wijmo {
     ...
     */
    export function showPopup(popup: HTMLElement, ref: any = null, above = false, fadeIn = false, copyStyles = true): any {
        ...
        let pos = new Point(),
            rc = null,
            scrWid = document.documentElement.clientWidth, //innerWidth,
        ...
        let position = 'absolute',
            ...
            ptOffset = new wijmo.Point(Math.max(0, -rcBody.left), Math.max(0, -rcBody.top));
        if (parent != document.body) {
            let op = popup.offsetParent,
                rcParent = op.getBoundingClientRect();
            ptOffset = new Point(op.scrollLeft - rcParent.left, op.scrollTop - rcParent.top);
        }
     ...
     */
    export function hidePopup(popup: HTMLElement, remove = true, fadeOut = false): any {
        let anim = null;
        if (fadeOut) {
            anim = animate(function (pct) {
